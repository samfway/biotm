...
### Load data
otu_file = '/Users/sawa6416/Projects/American-Gut/data/AG/AG_100nt.biom'
mapping_file = '/Users/sawa6416/Projects/American-Gut/data/AG/AG_100nt.txt'

prefix = 'AG_'
metadata_category = 'ALCOHOL_FREQUENCY'
...
sample_ids = sample_ids[keep] 
labels = labels[keep]
print 'Removing NA/None... Removed %d samples.' % (original_size - len(sample_ids))
print ""

### Combine and delete labels
combine = ['Never', 'Rarely (few times/month)']
...
combine = ['Daily', 'Regularly (3-5 times/week)']
new_label = label_legend.index(combine[0])  # Give everything the same label as the first element
for i in xrange(len(labels)):
    if label_legend[labels[i]] in combine:
        labels[i] = new_label
